...
/
  ...
  src/
    ...
    scripts/
      build-search-index.js (旧サイト保護のため残すが、Astro版では使用しない)
```
...
- **作業内容**: 
  ...
  - 検索インデックス生成エンドポイント（`src/pages/search-index.json.ts`）を実装。Content Collectionsのbodyを利用してビルドプロセス（`pnpm build`）内で自動的に `dist/search-index.json` を生成する。記事更新時に別コマンドを手動実行するような二重運用を排除する。
- **完了条件**: 
...
- **検索のビルド統合**:
  検索機能はAstroのContent Collectionsからインデックスをビルド時に自動生成する。インデックスの生成は `pnpm build` プロセスに含め、更新時に別コマンドを手動実行するような二重運用を排除する。旧 `build-search-index.js` は旧サイト保護のために残すが、Astro版では使用しない。
